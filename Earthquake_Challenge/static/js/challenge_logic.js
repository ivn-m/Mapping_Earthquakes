...
// DEFAULT LAYER: Create the map object with center, zoom level and default layer.
let map = L.map('mapid', {
  center: [39.5, -98.5],
  zoom: 3,
...
d3.json(earthquakesPast7Days).then(function (data) {
  ...
  // then we add the earthquale layer to the map
  earthquakes.addTo(map);

  // D2-3: Use d3.json to make a call to get our Tectonic Plate geoJSON data.
  d3.json("https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/4.5_week.geojson").then(function (majorData) {
    console.log(majorData);

    // D2-4: Use same style as earthquake data
    function styleInfo(feature) {
      return {
        opacity: 1,
        fillOpacity: 1,
    ...
    //D2-5: Change color cunction to use only 3 colors for magnitude <5, 5<, and 6<
    function getColor(magnitude) {
      if (magnitude > 6) {
        return "#ea2c2c";
      }
  ...
  d3.json("https://raw.githubusercontent.com/fraxen/tectonicplates/master/GeoJSON/PB2002_boundaries.json").then(function (tectonicData) {
    ...
    //D1-4 Pass data to geoJSON() layer
    L.geoJSON(tectonicData, {
      color: "black",
      weight: 2.5
    }).addTo(tectonicPlates);

    //D1-5 Add tectonicData layer group variable to the map
